        ...
        ngx_rtmp_relay_target_t *target)
{
    ...
    if (ngx_rtmp_relay_copy_str(s->connection->pool, &ctx->url, &target->url.url)
            != NGX_OK)
    {
        return NULL;
    }
        ...
        for (; *cctx; cctx = &(*cctx)->next) {
            ...
            if ((*cctx)->name.len == name->len
                && !ngx_memcmp(name->data, (*cctx)->name.data,
                    name->len)
                && (*cctx)->url.len == target->url.url.len
                && !ngx_memcmp((*cctx)->url.data, target->url.url.data, target->url.url.len))
            {
    			ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                "Exist: relay: name='%V'  url='%V', ctx: name='%V' url='%V'",
                name, &target->url.url, &(*cctx)->name, &(*cctx)->url);
                return NGX_ERROR;
            }
        }
    }
