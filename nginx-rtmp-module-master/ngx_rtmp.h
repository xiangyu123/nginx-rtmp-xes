...
#define NGX_RTMP_USER_SWF_REQUEST       0x1a
#define NGX_RTMP_USER_SWF_RESPONSE      0x1b
#define NGX_RTMP_USER_BUFFER_END        31

#define NGX_RTMP_SWF_HASH_LEN           32
#define NGX_RTMP_SWF_HASH_HEAD          10
/* Chunk header:
 *   max 3  basic header
...
typedef struct {
    ...
    ngx_uint_t              swf_check_port;
    ngx_str_t               swf_analyser_path;
    ngx_str_t               swf_analyser_pid_path;
...
typedef struct ngx_rtmp_core_srv_conf_s {
    ...
    /* swf check items */
    ngx_flag_t              swf_check;
    ngx_str_t               swf_path;
    ngx_msec_t              swf_interval;
    ngx_array_t             swfarray;/*ngx_str_t*/
    ngx_url_t               *url;
    ngx_pool_t              *swf_pool;
...
typedef struct {
    ...
    /*swf check key */
    u_char                  swf_key[NGX_RTMP_SWF_HASH_LEN];
    unsigned                swf_set:1;
    unsigned                swf_checked:1;
...
ngx_int_t ngx_rtmp_send_bandwidth(ngx_rtmp_session_t *s,
        uint32_t ack_size, uint8_t limit_type);
ngx_int_t ngx_rtmp_send_swf_request(ngx_rtmp_session_t *s, uint32_t msid);
