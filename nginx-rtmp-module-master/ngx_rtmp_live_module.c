...
ngx_rtmp_live_join(ngx_rtmp_session_t *s, u_char *name, unsigned publisher)
{
    ...
    stream = ngx_rtmp_live_get_stream(s, name, 1);//publisher || lacf->idle_streams);
...
*/
    ...
    ngx_rtmp_update_real_bandwidth(&ctx->stream->bw_real, h->timestamp, h->mlen, s->connection->log);
