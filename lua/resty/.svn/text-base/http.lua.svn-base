...
local function _receive_status(sock)
    ...
    --ngx.log(ngx.ERR, "cc_proxy_lua(HPC_error): status_line:"..line)
...
local function _receive_headers(sock)
    ...
    repeat
        ...
        --ngx.log(ngx.ERR, "cc_proxy_lua(HPC_error): header_line:"..line)
...
local function _chunked_body_reader(sock, default_chunk_size)
    --ngx.log(ngx.ERR, "cc_proxy_lua(HPC_error): xxxxxxxxxxxxxxxxx   socket failed")
    return co_wrap(function(max_chunk_size)
        ...
        repeat 
            ...
            if max_chunk_size and remaining > 0 then
                ...
                --ngx.log(ngx.ERR, "_chunked_body_reader(HPC_error): xxxxxxxxxxx socket failed length:"..length)
            ...
            if length == 0 then
                --local str, err = sock:receive(512)
                --if str then
                --    ngx.log(ngx.ERR, "_chunked_body_reader(HPC_error): xxxxxreread the socket failed length:"..string.len(str))
                --end
            end
